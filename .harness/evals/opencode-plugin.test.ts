...
describe("OpenCode adapter normalization", () => {
  ...
  test("fails closed on malformed edits", () => {
    expect(() =>
      normalizeToolEvent("pre_edit", { tool: "write", sessionID: "s1", callID: "c1" }, {}, "/work")
    ).toThrow("no path");
  });
  ...
  test("chat.message appends one routing pointer to the last text part", async () => {
    ...
    const parts = [{ id: "prt_1", type: "text", text: "please debug this failing test" }];
  ...
  test("chat.message is a no-op on no match, no text parts, and marker dedupe", async () => {
    ...
    expect(deduped[0].text).toBe("debug this <!-- aios-skill-route:systematic-debugging --> again");
  });
