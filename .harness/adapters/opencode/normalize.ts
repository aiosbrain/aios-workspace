...
function parsePatch(patch: string) {
  ...
  for (const line of patch.split("\n")) {
    ...
    if (move) {
      if (!current || !move[1])
        throw new Error("OpenCode patch rename has no source or destination");
      const from = current;
      current = move[1];
...
export function normalizeToolEvent(
  ...
  if (["apply_patch", "patch"].includes(input.tool)) {
    ...
    return event === "pre_edit" ? { ...base, ...parsed } : { ...base, paths: parsed.paths };
  }
...
export function normalizeUserPromptEvent(
  cwd: string,
  sessionID: string,
  prompt: string
): NormalizedEvent {
