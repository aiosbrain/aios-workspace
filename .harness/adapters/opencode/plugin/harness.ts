...
function trace(root: string, policy: string, event: NormalizedEvent, outcome: number) {
  ...
  const result = spawnSync(
    "/bin/sh",
    [join(root, "hooks", "trace-event.sh"), policy, String(outcome)],
    {
      cwd: event.cwd,
      input: JSON.stringify(event),
      encoding: "utf8",
...
export const HarnessGuards: Plugin = async ({ client, directory, worktree }) => {
  ...
  const enforce = (policy: string, event: NormalizedEvent, formatting = false) => {
    ...
    if (result.status !== 0)
      throw new Error(result.reason || `${policy} could not evaluate the event`);
  };
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      try {
        ...
        if (action.action === "continue" && typeof action.reason === "string")
