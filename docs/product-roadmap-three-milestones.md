...
The [Verified Weekly Operator Loop](./prd-verified-weekly-operator-loop.md): a weekly closeout that collects local work signals, drafts a private brief and shareable digest, verifies both against evidence and tier policy, and lets the user approve local writes, Team Brain sync, and Linear/PM actions.
