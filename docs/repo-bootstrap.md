...
| `scripts/check-workflow-policy.mjs` | MANAGED | copied | Static policy over GitHub Actions workflows reachable from PR-like events (leak-gate-remediation-plan.md §5.1 item 3): no `secrets.*`, no `checks: write`/`statuses: write`, no `AIOS Security Gate` context, every `uses:` pinned to a 40-hex commit SHA, and no `pull_request_target` combined with PR checkout / artifact download / package install / `${{ }}` interpolation into a `run:` body. Reads the target-owned co-located `scripts/workflow-policy-allowlist.json`. Zero-dependency, so the seeded CI job runs it with no `npm ci`. It never writes the required `AIOS Security Gate` status — only the `aios-security-gate` App may. |
| `scripts/workflow-policy-rules.mjs` | MANAGED | copied | The rule catalogue, PR-reachability graph, and audit the gate runs; split from the entry point so the policy stays readable and directly unit-testable. Pure — it takes parsed documents as data and returns findings. |
| `scripts/workflow-policy-catalogue.mjs` | MANAGED | copied | The rule catalogue (id → why/fix) plus the PR-like trigger list and the minimum-justification length. Dependency-free data, so the reporting, audit and allowlist layers can all read it without importing each other. |
| `scripts/workflow-policy-allowlist.mjs` | MANAGED | copied | Validates the reviewed-waiver file: every entry must name one real rule, one accountable owner, and a justification of at least 40 characters. A waiver that cannot name a real rule is a failure regardless of what any workflow contains. |
| `scripts/workflow-policy-expressions.mjs` | MANAGED | copied | Decides what counts as attacker-controlled: the provably-trusted field allowlist, the transport/archive primitives, and taint resolution through `${{ env.X }}`, `$SHELL_VAR` and `${{ inputs.X }}`. Every bypass found in adversarial review of the gate originated here, so it is kept as one small separately-readable unit. |
| `scripts/workflow-yaml.mjs` | MANAGED | copied | The strict, fail-closed YAML reader `check-workflow-policy.mjs` parses workflow files with. Throws on any construct it does not model (anchors, aliases, merge keys, tags, multi-document, duplicate keys) so an unreadable workflow is a violation, never a skip. |
| `scripts/git-files.mjs` | MANAGED | copied (resolved body) | Shared git-enumeration helper both gates import. Sourced from `packages/foundation/src/git-files.mjs` — the toolkit's own `scripts/git-files.mjs` is a relative-path shim into `packages/` (AIO-601) that would dangle in a target with no `packages/` tree, so the self-contained module body is stamped at the import path instead. |
...
| `scripts/workflow-policy-allowlist.json` | SEED | generated → **independently-owned** | Empty waiver list for the workflow-policy gate. A bootstrapped repo starts with zero exceptions; each later entry names one `(workflow, job, rule)` triple, an `owner`, and a justification of at least 40 characters. |
| `.github/workflows/ci.yml` | SEED | generated (parameterized) → **independently-owned** | Four jobs: governance gates (size + boundaries), **workflow policy**, lint, tests. `--lint-script`/`--test-script` fill the npm script names (`--if-present`, tolerant of a repo with no `package.json` yet). Workflow-level `permissions: contents: read`, every action pinned by commit SHA, and — since the leak-gate remediation (§5.1 item 4) — **no secret in any job**: the seeded CI no longer generates a `pull_request`-triggered confidentiality scanner reading `AIOS_LEAK_TERMS_B64`. Confidentiality scanning is the advisory local pre-push hook plus, once it exists, the `aios-security-gate` App. Enrolling a new repo with that App and its ruleset is a documented TODO in the template, not generated configuration. |
...
| `.git/hooks/{pre-commit,pre-merge-commit,reference-transaction,pre-push,post-checkout}` | install action | installed | Installed **from the target's own stamped copies**; pre-existing foreign hooks are chained (`<hook>.chained`), a foreign `post-checkout` is preserved untouched. In a `core.hooksPath` repo whose destination hook carries the line-anchored `# aios-tracked-hook` marker (a version-controlled policy hook), the tracked file is left byte-identical and the machine-local hook goes to `$(git rev-parse --git-common-dir)/hooks/` — the chain target the tracked hook execs (AIO-638). Required hook names without a tracked dispatcher remain executable in `core.hooksPath` but are added as exact machine-local paths to `.git/info/exclude`, so they do not dirty the checkout. hooksPath **without** the marker keeps the legacy install into the hooksPath dir. |
