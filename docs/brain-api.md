...
- *2026-07-30 — **v1.15**: added an optional **`codebase_health`** object under `metrics` on
  ...
  [`contract/codebase-payload-1.24-fixtures.json`](./contract/codebase-payload-1.24-fixtures.json).
...
- *2026-08-31 — **v1.24**: `POST /api/v1/codebases` gains two OPTIONAL, nullable scanner-identity
  ...
  **Compatibility, both directions, stated semantically rather than on the wire.** An OLD scanner
  against a 1.24 brain: accepted unchanged, both fields null, every score identical — it renders
...
- **Scanner identity** (document revision 1.24, **both optional and nullable**, AIO-1011).
  ...
  - **A sender SHOULD send `scanner_version` and MUST NOT fabricate it.** If the build cannot be
    ...
    to report".
  - **`null` means UNKNOWN, and a reader MUST NOT treat it as current.** Every pre-1.24 scan is in
  ...
  - **The comparison is a declared minimum, not a commit distance.** See the v1.24 revision entry
...
Canonical source bytes and SHA-256 are pinned from merged Harness commit `691762a469f5e816d35a6263467c9e2d8cc98e8e`: `../aios-engineering-harness/evals/FINDING_OBSERVATIONS_CONTRACT.md` SHA-256 `b23baa16b0055e4841584433390c9412ac11c922a333dec35cae45108761e0ac`, and `../aios-engineering-harness/evals/schemas/finding-observations.v1.schema.json` SHA-256 `a51f360769ab26440287891867447baf6e3df93073df6f0d7febc1f3c10322b2`; the latter is vendored into Workspace contract assets. Do not invent an alternative event object or alter its identity algorithm. The request has exactly `{schema_version:"debt-intake-events.v1", events:[...]}`. Each event is the unchanged canonical union of `candidate` and `run_summary`; producer/schema versions are already required on every record. A finalized zero-candidate run sends its canonical summary, never an empty events list. Missing links remain explicit null. Requests use application/json UTF-8, with 1..256 records and a 1 MiB raw-body ceiling checked before full buffering; excess records returns 422 invalid_events and excess bytes 413 payload_too_large. Reject duplicate JSON keys, invalid UTF-8, BOM, non-finite/decimal/exponent numeric tokens, unknown fields, invalid calendar timestamps, identity mismatches, and canonical semantic violations with 422 invalid_events. The JSON envelope has no JSONL newline constraints; individual records retain all normative value and canonicalization constraints.

The same append-only event table stores run summaries with nullable candidate identity; a CHECK distinguishes candidate versus summary records. A run is keyed by `(team_id, producer.name, producer.run_id)` and has at most one logical summary. A summary is only accepted when its counts reconcile against ALL stored plus incoming events of that producer run and canonical prior history. Candidate records may arrive in bounded batches before a summary; they remain explicitly unfinalized/unknown completeness in the read model until a summary is accepted. No batch boundary, absence, age, or retry implies completion. Once a run summary is accepted, new non-replay records for that run are rejected. A later lifecycle stage uses a new producer run and summary. Empty complete, partial, unknown, and malformed-accounted summaries preserve the UH3-5 denominator semantics verbatim. This resolves how an incremental API ingests the canonical finalized-ledger contract without silently dropping its summaries.

### Authorization and replay

Authentication determines team and actor; neither is accepted from the request. An active team-tier principal with write access is required; admin/private and external principals do not gain access implicitly. Unauthenticated requests return 401; external tier returns 403 forbidden_tier; inaccessible or unknown routing codebase returns indistinguishable 404 not_found. The `:slug` is an authorized routing anchor, not a namespace for event/candidate identity. Every incoming candidate membership and every typed link independently resolves to authorized same-team registry entries. Cross-repo candidates keep ONE team-scoped candidate identity. A run can cover several authorized codebases; do not force every candidate to name the anchor, which would prevent ingesting mixed-repository runs. Summary-only requests are allowed (empty capture); their routing anchor is audit context, not fabricated candidate membership. Trusted producer versions, codebases and Linear keys come from reviewed server configuration and remain available for historical replay.

Before mutation, validate canonical bytes and hash identities. Within a single transaction serialize the team ledger (implementation may use a team-scoped transaction advisory lock), read the needed candidate/run/duplicate history, validate the union of persisted plus incoming records, upsert identities/memberships, insert new records, and append a payload-free audit. Candidate identity is immutable. Membership rows are historical union, while current memberships come from the latest canonical event; removal is never inferred from absent batches. `(team_id,event_id)` is unique; an existing ID with different canonical bytes rejects the WHOLE batch, including concurrent conflicts. `(team_id,candidate_id,sequence)` also forbids divergent forks. Exact duplicate occurrences in one request count once; acknowledgments contain distinct first-occurrence-ordered IDs. IDs inserted by this request appear only in accepted_event_ids; preexisting IDs appear only in duplicate_event_ids; the sets are disjoint. A replay-only acknowledgment is 200; any new event, including a new summary, yields 201. The closed acknowledgment is exactly `{status:"ok", accepted_event_ids:string[], duplicate_event_ids:string[]}`. No rejected success bucket and no partial persistence. Audit failure rolls back the batch; acknowledgment is sent only after durable commit.

Validate contiguous sequence/predecessors, frozen discovery identity/program attribution, legal state/disposition/episode/evidence/link transitions, complete historical duplicate-target existence and cycle freedom, producer-run attribution consistency, and run-summary reconciliation against stored plus incoming history. Array/input order is irrelevant. A later event lacking a required predecessor fails 422 and may be retried after predecessors are ingested; it is never staged as a valid candidate. Lock timeout is bounded and returns retryable 503 intake_busy with Retry-After; rate limiting follows the existing API limiter and returns 429 with Retry-After. Old servers return 404; the future publisher retains local pending records and reports unsupported intake, never falls back to snapshot/item ingest.

### Authenticated producer provenance

Team membership alone does not authorize finding ingestion. Reviewed server configuration explicitly binds the authenticated API key or service principal to permitted producer namespaces and versions. Unconfigured producer-principal combinations return 403 forbidden_producer with no ledger writes. Historical registry entries remain available for replay validation, but do not restore revoked upload authority. Ordinary team writer credentials cannot impersonate an approved producer or finalize a fabricated complete-zero run.

The transfer trust boundary is a dedicated, namespace-bound uploader credential: successful submission is that uploader's authenticated attestation, not evidence inferred by the server from a digest. Before submission the trusted uploader must use the pinned producer validator to verify detector evidence against the retained sanitized inventory, raw counts, capture/completion result, and canonical ledger. The uploader retains this sanitized evidence privately keyed by its evidence digest until rollout verification and the team's reviewed retention policy permit disposal; no raw content is transmitted or retained for this purpose. Missing or unverifiable evidence blocks publishing a complete summary; the uploader must preserve the local failure/pending result without silently rewriting the canonical summary or claiming complete capture. The Brain independently validates structure, registry, hashes, lifecycle, and reconciliation, while authenticated uploader identity is the explicit source of external inventory attestation. This distinction is documented as a trust assumption, not a claim the server can prove inventory truth from hashes. Readback/reconciliation during AIO-1102 and rollout must recheck the retained sanitized inventory against stored summary hashes and counts. No fields are added to canonical records.

Normative fixtures cover ordinary team-key producer impersonation denial, explicitly authorized uploader acceptance, revoked uploader replay denial, and complete-zero finalization without verified inventory evidence blocked by uploader preflight. The latter is a required future publisher outcome, not a claimed server-side ability to recover absent inventory from its digest. Production activation requires that uploader control and its retained-evidence reconciliation proof.
