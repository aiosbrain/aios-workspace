...
| Unified Inbox (AIO-381 / I-01) | [unified-inbox.md](./unified-inbox.md) · [overview](./unified-inbox-overview.md) | **Net-new** meta-inbox over agent asks + external comms: `inbox-events.ndjson` journal, SQLite read model (D5), orthogonal state machines, runtime-issued capability handle, reply PDP, enriched observations, audit anchor (D6), pre-registered metrics |
...
| Test-coverage → Brain (AIO-531) | [test-coverage-brain.md](./test-coverage-brain.md) | **Net-new** optional `test_coverage` scalars on `POST /api/v1/metrics` (brain-api 1.13, sibling of `context_health`): zero-dep producer + third non-axis shadow card + scalars-only push; shadow-first, two PRs |
...
| Property-based testing (AIO-532) | [property-testing.md](./property-testing.md) | **Net-new** first-wave fast-check properties (tier resolution/default-deny, toolkit 3-way merge, item-payload schema↔validator parity, capability broker) + replayable `test/helpers/arbitraries.mjs`; devDep-only, mutation-lane linked |
| Mutation denominator (AIO-539) | [mutation-denominator.md](./mutation-denominator.md) | **Config-only** fix for a nightly that has never completed: scope the access-governance campaign to the `buildPlan` safety unit (3,503 → ~625 mutants), matrix the nightly one job per group so no group starves the other five, and raise the changed-code lane timeout so it stops reporting green by timing out |
| Safety-unit extraction wave 1 (AIO-540) | [safety-unit-extraction.md](./safety-unit-extraction.md) | **Net-new** `scripts/sync-plan.mjs` — moves the sync-plan safety gate (`buildPlan` + push state) out of the 2,519-line `scripts/aios.mjs` so the mutation target is a module boundary instead of a line range; behavior-preserving, sequenced after Mutation denominator |
