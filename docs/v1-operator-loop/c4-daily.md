...
### Boundaries
- **The only write** C4 performs is the local change-snapshot under `.aios/loop/state/` — the same local-only, never-synced class as run manifests (outside `sync_include`; see `manifest-nonsync.test.mjs`). It records **only** on an owner run; `--as`, `--manifest`, and `--no-record` runs are fully side-effect-free.
