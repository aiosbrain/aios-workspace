...
This workspace ships **7** skill(s). Skills activate automatically when their description matches what you ask; you can also invoke one by name.
...
| `workspace-setup` | Set up (or update) the owner's profile in this workspace: interview the user about who they are and what they're working on, then write it into the workspace memory files (.claude/memory/USER.md + WORKSPACE.md) so the agent has durable context. |
