...
const SESSION = [
  { type: "user", sessionId: "s1", uuid: "u1", parentUuid: null, timestamp: "2026-06-10T10:00:00Z", cwd: "/home/me/proj",
    message: { role: "user", content: "please run the tests" } },
  { type: "assistant", sessionId: "s1", uuid: "a1", parentUuid: "u1", timestamp: "2026-06-10T10:00:05Z", gitBranch: "main", cwd: "/home/me/proj",
    message: { role: "assistant", model: "claude-opus-4-8", usage: usage({ input_tokens: 1000, output_tokens: 500, cache_read_input_tokens: 8000, cache_creation_input_tokens: 200 }),
      content: [ { type: "text", text: "ok" }, { type: "tool_use", name: "Bash" } ] } },
  { type: "user", sessionId: "s1", uuid: "u2", parentUuid: "a1", timestamp: "2026-06-10T10:00:09Z",
    message: { role: "user", content: [ { type: "tool_result", is_error: false } ] } },
  { type: "assistant", sessionId: "s1", uuid: "a2", parentUuid: "u2", timestamp: "2026-06-10T10:00:12Z", gitBranch: "main", cwd: "/home/me/proj",
    message: { role: "assistant", model: "claude-opus-4-8", usage: usage({ input_tokens: 500, output_tokens: 300, cache_read_input_tokens: 9000 }),
      content: [ { type: "text", text: "tests pass" } ] } },
  // a subagent turn (delegation signal)
  { type: "assistant", sessionId: "s1", uuid: "a3", parentUuid: "a2", isSidechain: true, timestamp: "2026-06-10T10:00:20Z", cwd: "/home/me/proj",
    message: { role: "assistant", model: "claude-opus-4-8", usage: usage({ input_tokens: 200, output_tokens: 100 }),
...
console.log("normalize");
{
  ...
  check("normalizeTokens maps long keys → short", normalizeTokens(usage({ input_tokens: 5 })).in === 5);
  check("totalTokens sums four buckets", totalTokens({ in: 1, out: 2, cache_read: 3, cache_create: 4 }) === 10);
  check("makeEvent rejects bad tool", (() => { try { makeEvent({ tool: "nope" }); return false; } catch { return true; } })());
  check("makeEvent defaults actor to assistant", makeEvent({ tool: "claude" }).actor === "assistant");
}
...
const events = recordsToEvents(SESSION, "s1");
{
  ...
  check("tool_use events captured with names", toolUses.length === 2 && toolUses.some((t) => t.tool_name === "Bash"));
  check("tool_result event captured (not a task root)", toolResults.length === 1);
  check("isSidechain → subagent actor (turn + its tool_use)", subagent.length === 2 && subagent.some((e) => e.tool_name === "Grep"));
  check("permission-mode → permission event", events.some((e) => e.block_type === "permission"));
  check("ai-title noise ignored", !events.some((e) => e.block_type === "meta" && e.tool_name === "x"));
  check("usage attached once per assistant turn (3 token events)", tokenEvents.length === 3);
  check("git branch + project threaded through", events.some((e) => e.git_branch === "main" && e.project === "proj"));
  check("parseClaude(text) parity with records path", parseClaude(SESSION.map((r) => JSON.stringify(r)).join("\n"), "s1").length === events.length);
}
...
const sig = computeSignals(events);
{
  ...
  check("cache-hit excludes accumulated reads from denom", approx(sig.cache_hit_rate, 17000 / (17000 + 1700 + 200)));
...
console.log("aem — scoring + the verification gate");
{
  ...
  const strong = { verify_tool_rate: 0, cache_hit_rate: 0.8, delegation_ratio: 0.3, subagent_usage: 0.5,
    tool_diversity: 8, tokens_per_task: 10_000, permission_events: 5, correction_loop_avg: 1, error_rate: 0 };
  const axesWeakVerify = scoreAxes(strong);
  check("verification scores 0 when no verify tools", axesWeakVerify.verification === 0);
  check("GATE: Spine capped at L3 when verification ≤ 1", spineLevel(axesWeakVerify, strong) === "L3");
  ...
  check("placement returns axes + spine + overall + weakest", !!p.axes && !!p.spine && typeof p.overall === "number" && !!p.weakest);
  check("learning axis capped at 3 (proxy limitation)", scoreAxes({ ...strong, tool_diversity: 50 }).learning === 3);
}

console.log("================================================");
if (failed === 0) { console.log(`${GREEN}analyze tests PASSED${NC}`); process.exit(0); }
console.log(`${RED}analyze tests FAILED — ${failed} assertion(s)${NC}`); process.exit(1);
