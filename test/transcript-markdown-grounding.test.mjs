...
test("stripTranscriptMarkup removes frontmatter and speaker labels", () => {
  ...
  assert.equal(
    stripTranscriptMarkup(raw),
    "Hello there. Sounds good."
  );
});

test("isOrderedTokenSubsequence accepts quotes split across speaker turns", () => {
  const transcript = [
  ...
  const quote =
    "So it's important as well, John, that we put those icons on the front page.";
  assert.equal(isOrderedTokenSubsequence(quote, transcript), true);
  assert.equal(isNearVerbatim(quote, transcript), true);
});

test("isOrderedTokenSubsequence accepts cross-turn quotes with intervening speaker labels", () => {
  const transcript = [
