...
test('search sends authorized structured request and preserves bounded evidence',async()=>{
  const data={sources:[{sid:'S1',item_id:'one',excerpt:'quote',contributors:[]}],returned:1,truncated:false};
  const client=createBrainClient(cfg,{fetch:async(url,options)=>{assert.equal(url,'https://brain.example/api/v1/evidence/search');assert.equal(options.headers.Authorization,'Bearer synthetic-key');assert.deepEqual(JSON.parse(options.body),{query:'question',limit:8});return Response.json(data);}});
  const tool=TOOLS.find(t=>t.name==='brain_search_evidence');assert.ok(tool);
...
test('older servers require upgrade and errors never become empty success',async()=>{
  for(const status of [401,403,404,405,429,500]){
    const client=createBrainClient(cfg,{fetch:async()=>Response.json({error:{code:'failed',message:'failed'}},{status})});
    await assert.rejects(()=>client.searchEvidence('question'),status===404||status===405 ? /upgrade/i : new RegExp(String(status)));
  }
});
test('malformed and oversized success responses are rejected',async()=>{
 for(const data of [{}, {sources:[],returned:0,truncated:false,padding:'x'.repeat(21000)}]){
...
test('direct-client cancellation reaches the authenticated fetch',async()=>{
 ...
 const client=createBrainClient(cfg,{fetch:async(_url,options)=>new Promise((_,reject)=>options.signal.addEventListener('abort',()=>reject(options.signal.reason),{once:true}))});
