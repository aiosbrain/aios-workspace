 ...
 * classified into one effective bucket: MANAGED (3-way synced), SEED_IF_ABSENT
 * (create-only), PERSONAL (never overwritten), or SCAFFOLD_UNMANAGED (written once,
 * deliberately not synced). This test fails when someone adds a new
 * `"$OUTPUT/<path>"` destination to
...
/** Seeds are narrower than personal parents: file seeds cover only their exact destination. */
function seedCovers(entry, d) {
  return entry.kind === "dir"
    ? d === entry.dest || d.startsWith(entry.dest + "/")
    : d === entry.dest;
}
...
test("every scaffold-written toolkit path is classified (manifest ↔ scaffold parity)", () => {
  ...
  assert.deepEqual(
    ...
    `scaffold writes these paths but the manifest doesn't classify them ` +
      `(add to MANAGED_PATHS, SEED_IF_ABSENT, PERSONAL_PATHS, or SCAFFOLD_UNMANAGED): ${unclassified.join(", ")}`
  );
});
...
test("the four manifest buckets have no unsafe overlap", () => {
  ...
  for (const seed of SEED_IF_ABSENT) {
    assert.ok(
      !MANAGED_PATHS.some((e) => managedCovers(e, seed.dest)),
      `${seed.dest} is both seed-if-absent and managed`
    );
    assert.ok(
      !SCAFFOLD_UNMANAGED.some(
        (p) => p === seed.dest || seed.dest.startsWith(p + "/") || p.startsWith(seed.dest + "/")
      ),
      `${seed.dest} is both seed-if-absent and scaffold-unmanaged`
    );
    const personalOverlaps = PERSONAL_PATHS.filter(
      (p) => p === seed.dest || seed.dest.startsWith(p + "/") || p.startsWith(seed.dest + "/")
    );
    assert.ok(
      personalOverlaps.every((p) => seed.dest.startsWith(p + "/")),
      `${seed.dest} may only overlap as a narrow child of a personal directory`
    );
  }
});

test("the comms config is a create-only seed inside personal .aios state", () => {
  const seed = SEED_IF_ABSENT.find((e) => e.dest === ".aios/comms-config.json");
  assert.ok(seed, "comms config seed is declared");
  assert.equal(seed.kind, "file");
  assert.ok(PERSONAL_PATHS.includes(".aios"), "the surrounding .aios state remains personal");
  assert.ok(scaffold.includes(`"$OUTPUT/.aios/comms-config.json"`));
});
