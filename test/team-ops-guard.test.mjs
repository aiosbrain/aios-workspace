...
test("shared and standalone hooks treat a private-key pattern as data, not a grep option", () => {
  const workspace = mkdtempSync(path.join(tmpdir(), "aios-standalone-guard-"));
  try {
    const hooksDir = path.join(workspace, "hooks");
    mkdirSync(hooksDir, { recursive: true });
    const standaloneHook = path.join(hooksDir, "team-ops-guard.sh");
    copyFileSync(SOURCE_HOOK, standaloneHook);
    ...
    const payload = JSON.stringify({
      ...
      tool_input: {
        file_path: "2-work/report.md",
        edits: [{ old_string: "before", new_string: marker }],
      },
    });

    for (const hook of [SOURCE_HOOK, standaloneHook]) {
      const result = spawnSync("bash", [hook], {
        cwd: hook === SOURCE_HOOK ? REPO : workspace,
        input: payload,
        encoding: "utf8",
      });
      assert.equal(result.status, 2, result.stderr);
