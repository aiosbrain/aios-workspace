...
async function startMcp(key, t) {
  ...
  const child = spawn(
    process.execPath,
    [fileURLToPath(new URL("../scripts/brain-mcp.mjs", import.meta.url))],
    {
      cwd,
      env: {
        PATH: process.env.PATH,
        HOME: cwd,
        AIOS_BRAIN_URL: fixture.url,
        AIOS_API_KEY: key,
...
async function initialize(rpc) {
  const response = await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "tier-safety", version: "1" },
  });
...
test("live MCP authorization boundary", { timeout: 120000 }, async (t) => {
  ...
  try {
    await t.test("project-route denial survives a stale team tool list", async (t) => {
      ...
      const projects = data(
        await rpc("tools/call", { name: "brain_list_projects", arguments: {} })
      );
      ...
      assert.ok(
        (await rpc("tools/list")).result.tools.some((tool) => tool.name === "brain_list_projects")
      );
      const result = await rpc("tools/call", { name: "brain_list_projects", arguments: {} });
      assert.equal(
        result.result.isError,
        true,
        "MCP_PROJECT_DENIAL: Brain must deny the stale board tool"
      );
    ...
    await t.test("item visibility follows explicit grants and revocation", async (t) => {
      ...
      async function check(expected) {
        ...
        assert.deepEqual(
          paths,
          [...expected].sort(),
          "MCP_ITEM_VISIBILITY: exact nonempty authorized fixture membership"
        );
