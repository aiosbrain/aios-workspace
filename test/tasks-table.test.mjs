...
check("six-column parses row_key/title/status", sixRows.length === 1 && sixRows[0].row_key === "T-01" && sixRows[0].status === "in_progress");
check("six-column emits NO hierarchy keys", !("parent" in sixRows[0]) && !("labels" in sixRows[0]) && !("priority" in sixRows[0]));
...
check("hierarchy parses labels as array", Array.isArray(hierRows[0].labels) && hierRows[0].labels.length === 2 && hierRows[0].labels[0] === "integration");
...
const m1 = mergeTaskWriteback(baseTable, [
  { row_key: "T-01", title: "New title", assignee: "alex", status: "done", sprint: "s1", due: null },
  { row_key: "T-02", title: "Fresh", assignee: "sam", status: "ready", sprint: "s2", due: null },
]);
check("update in place (T-01 title changed)", /\| T-01 \| New title \|/.test(m1) && !/Old title/.test(m1));
...
const m2 = mergeTaskWriteback(baseTable, [
  { row_key: "T-01", title: "Epic", assignee: "alex", status: "in_progress", sprint: "s1", due: null, parent: null, labels: ["frontend", "ui"], priority: "high" },
]);
check("header upgraded with Parent|Labels|Priority", /\| ID \| Task \| Assignee \| Status \| Sprint \| Due \| Parent \| Labels \| Priority \|/.test(m2));
check("separator row widened to 9 cols", (m2.split("\n").find((l) => /^\|\s*---/.test(l)).match(/---/g) || []).length === 9);
...
const m3 = mergeTaskWriteback(baseTwoRows, [
  { row_key: "T-01", title: "A", assignee: "a", status: "done", sprint: "s1", due: null, parent: null, labels: [], priority: "low" },
]);
...
const m4 = mergeTaskWriteback(baseTable, [{ row_key: "T-01", title: "Cost is $5 & $0.50", assignee: "", status: "done", sprint: "", due: null }]);
...
`;
const m5 = mergeTaskWriteback(pmTable, [{ row_key: "T-01", title: "x", assignee: "a", status: "done", sprint: "s1", due: null, pm_provider: "plane", pm_external_id: "T-01", pm_url: "http://x" }]);
