...
if (!repo) { console.error("Usage: check-skill-export.mjs <repo>"); process.exit(1); }
...
for (const rt of RUNTIMES) {
  ...
  if (!skillDirs.length) { fail(`${rt} — no skills emitted`); rmSync(out, { recursive: true, force: true }); continue; }

  if (rt === "claude-code") {
    // identity copy must preserve the multi-agent harness executable
    if (!files.some((f) => f.endsWith(".workflow.js"))) fail("claude-code — .workflow.js not preserved");
    ...
    for (const f of files.filter((f) => path.basename(f) === "SKILL.md")) {
      ...
      if (!desc || /^["']?[|>][-+]?["']?$/.test(desc)) fail(`${rt}/${name} — empty/block-scalar description`);
  ...
  if (!errors) console.log(`  ${GREEN}✓${NC} ${rt} — ${skillDirs.length} skill(s) exported & validated`);
  rmSync(out, { recursive: true, force: true });
}

console.log("================================================");
if (errors === 0) { console.log(`${GREEN}OGR06 PASSED${NC}`); process.exit(0); }
console.log(`${RED}OGR06 FAILED — ${errors} issue(s)${NC}`); process.exit(1);
