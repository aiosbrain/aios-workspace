...
import type { BigTextFont } from "./big-text.js";
...
const SetupFlowStep = ({ icon, iconColor, status = "done", children }: SetupFlowStepProps) => {
  const unicode = useUnicode();
  const theme = useTheme();
  const { asciiIcon, icon: defaultIcon, dim } = STATUS_ICONS[status];
...
const SetupFlowSpinner = ({ label, isActive = true }: SetupFlowSpinnerProps) => {
  const theme = useTheme();
  const unicode = useUnicode();
