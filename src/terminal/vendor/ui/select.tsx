...
export const Select = <T = string,>({
  ...
  const [activeIndex, updateActiveIndex] = useState(() => {
    const selected = options.findIndex(
      (option) => !option.disabled && option.value === (controlledValue ?? defaultValue)
    );
    return selected < 0 ? firstEnabledIndex : selected;
  });
  ...
  const setActiveIndex = (next: number | ((current: number) => number)) => {
    ...
    updateActiveIndex(activeRef.current);
  };
      ...
      {options.map((opt, idx) => {
        ...
        return (
          <Box
            ...
            gap={1}
          >
            <Text aria-hidden color={isActive ? resolvedCursorColor : undefined}>
