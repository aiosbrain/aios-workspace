...
const FocusScopeContext = React.createContext<FocusScopeContextValue | null>(null);
...
export const FocusScope = ({
  ...
  const register = React.useCallback((control: RegisteredControl) => {
    const existingIndex = controlsRef.current.findIndex(({ id }) => id === control.id);
    if (existingIndex === -1) {
    ...
    return () => {
      controlsRef.current = controlsRef.current.filter(({ id }) => id !== control.id);
      setRegistrationVersion((version) => version + 1);
    };
  }, []);

  const setNestedActive = React.useCallback((nestedScopeId: string, nestedActive: boolean) => {
    if (nestedActive) {
  ...
  React.useEffect(() => {
    ...
    restoreFocusIdRef.current = returnFocusId ?? lastFocusedByStdout.get(stdout) ?? null;
    disableFocus();
  ...
  React.useEffect(() => {
    ...
    setFocusedId((currentId) => {
      ...
      return initialFocusId && enabledIds.includes(initialFocusId) ? initialFocusId : enabledIds[0];
    });
  ...
  return <FocusScopeContext.Provider value={contextValue}>{children}</FocusScopeContext.Provider>;
};
...
export function useInteraction(
  ...
  const options =
    typeof optionsOrHandler === "function"
      ...
      : optionsOrHandler;
  const { id: providedId, autoFocus = false, isActive = true, disabled = false, onInput } = options;
