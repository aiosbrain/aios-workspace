...
if (!/^[a-f0-9]{40}$/.test(sha || '')) throw new Error('MCP_BRAIN_SHA must be an explicit full commit SHA');
...
const env = { PATH: process.env.PATH, HOME: scratch, CI: '1', NEXT_TELEMETRY_DISABLED: '1',
...
function run(command, args, { cwd = scratch, extraEnv = {}, allowFailure = false, timeout = 600000, cleanup = false } = {}) {
  ...
  return new Promise((resolveRun, reject) => {
    ...
    child.once('close', (code) => {
      ...
      if (code !== 0 && !allowFailure) reject(new Error(`${command} ${args.join(' ')} failed (${code})\n${output.slice(-12000)}`));
...
const report = { brainSha: sha, workspaceSha: (await run('git', ['rev-parse', 'HEAD'], { cwd: workspace })).output.trim(), runs: [] };
let failed;
try {
  await run('docker', ['info']);
  const resolved = (await run('git', ['rev-parse', `${sha}^{commit}`], { cwd: brain })).output.trim();
  ...
  for (const mutation of ['baseline', 'project-denial', 'item-visibility']) {
    ...
    await symlink(await realpath(join(brain, 'node_modules')), join(directory, 'node_modules'), 'dir');
    let mutationDigest = null;
    if (mutation !== 'baseline') {
      const target = mutation === 'project-denial' ? 'app/api/v1/projects/route.ts' : 'app/api/v1/items/route.ts';
      const before = mutation === 'project-denial' ? 'if (auth.memberTier !== "team") {' : 'q = q.in("id", [...ids]);';
      const after = mutation === 'project-denial' ? 'if (false) {' : '// AIO-1109 disposable mutation: omit item visibility intersection';
    ...
    try {
      ...
      const build = await run(process.execPath, ['node_modules/next/dist/bin/next', 'build'], { cwd: directory, extraEnv, allowFailure: true });
      ...
      const test = await run(process.execPath, ['node_modules/vitest/vitest.mjs', 'run', '--config', 'vitest.mcp.config.ts'], { cwd: directory, extraEnv, allowFailure: true, timeout: 240000 });
