...
const TEXT_RE = /\.(md|txt|py|mjs|cjs|js|jsx|ts|tsx|sh|bash|zsh|rb|go|pl|php|ps1|json|ya?ml|toml|cfg|ini|html?|xml|env)$/i;
...
const RULES = [
  // — network egress —
  { rule: "network-egress", severity: "high", where: "code",
  ...
  { rule: "network-egress-shell", severity: "high", where: "any",
    re: /\b(curl|wget|netcat)\b\s+\S|\bnc\b\s+-|\b(scp|ftp|telnet)\b\s+\S/ },
  // — filesystem / process execution —
  { rule: "process-exec", severity: "info", where: "code",
  ...
  { rule: "exfil-encode", severity: "info", where: "code",
...
const URL_ALLOW = /^https?:\/\/([a-z0-9-]+\.)*(anthropic\.com|claude\.com|github\.com|githubusercontent\.com|npmjs\.com|python\.org|pypi\.org|modelcontextprotocol\.io|w3\.org|apache\.org|mozilla\.org)(\/|$|:)/i;
...
function scanText(file, text, findings, { isSkillMd }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (hidden) {
      findings.push({ file, line: ln, rule: "hidden-unicode", severity: "high",
        snippet: `${codePoint(hidden[0])} hidden char — ${snippet(line)}` });
    }

    // Prompt-injection phrasing.
    for (const re of INJECTION_PHRASES) {
      if (re.test(line)) { findings.push({ file, line: ln, rule: "prompt-injection", severity: "high", snippet: snippet(line) }); break; }
    }
    ...
    for (const r of RULES) {
      ...
      if (r.re.test(line)) findings.push({ file, line: ln, rule: r.rule, severity: r.severity, snippet: snippet(line) });
    }

    // External URLs in SKILL.md (non-allowlisted host) — high (a fetch target the user can't see).
    if (isSkillMd) {
      const urls = line.match(URL_RE);
      if (urls) for (const raw of urls) {
        const u = raw.replace(/[.,;:!?]+$/, ""); // drop trailing sentence punctuation
        if (!URL_ALLOW.test(u)) findings.push({ file, line: ln, rule: "external-url", severity: "high", snippet: u.length > 120 ? u.slice(0, 117) + "…" : u });
      }
    }
  }
}
...
export function scanSkill(dir) {
  ...
  const counts = { total: findings.length, high: findings.filter((f) => f.severity === "high").length, code_files: codeFiles.length };
...
function main() {
  ...
  if (!dir) { console.error("usage: node scripts/skill-scan.mjs <skill-dir> [--json]"); process.exit(2); }
  const res = scanSkill(dir);
  if (json) { console.log(JSON.stringify(res, null, 2)); return; }
  console.log(`risk: ${res.riskClass}  (${res.counts.high} high-severity of ${res.counts.total} findings; ${res.counts.code_files} code files)`);
  for (const f of res.findings) console.log(`  [${f.severity}] ${f.file}:${f.line}  ${f.rule}  — ${f.snippet}`);
