...
| Built-in adapters | `connectors.mjs` (lazy barrel — the ONLY import route into `connectors/`) + `connectors/linear/` (the one Linear implementation behind `aios linear`, AIO-1067) + `connectors/slack/` (the one Slack implementation behind `aios slack`, AIO-1068 — no Python path remains); `linear.mjs` and `slack.mjs` are the warning-only compat delegates |
