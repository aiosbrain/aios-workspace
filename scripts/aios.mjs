...
async function cmdPush(repo, cfg, patterns, args) {
  ...
  // A per-item Brain rejection is a failed push even though we continue through the
  // remaining items and preserve their individual outcomes. Propagate that partial
  // failure to shell callers (including the GUI's execFile bridge) instead of exiting
  // zero and letting the cockpit report a false success.
  if (result.failed.size) process.exitCode = 1;
  if (plan.blocked.length)
    console.log(c.dim(`${plan.blocked.length} blocked — run 'aios status' for reasons.`));
