...
// walkFiles / loadState / saveState / contentShaForPush / buildPlan now live in
// ./sync-plan.mjs (AIO-540) — the sync-plan safety gate is its own module so the
// access-governance mutation group has a real boundary to mutate. Imported above.
...
/**
 * The v1 handler/resolver bundle. The canonical entrypoint imports this module only after
 * dispatch has selected a non-diagnostic command, so help/version/doctor/provenance never load
 * the legacy config, credential, connector, or provider graph.
 */
