...
} from "./internal/brain-origin.mjs";
...
export function createBrainClient(config, deps = {}) {
  ...
  async function searchEvidence(query, project, limit = 8, { signal } = {}) {
    if (typeof query !== "string" || !query.trim() || query.trim().length > 2000 ||
        !Number.isInteger(limit) || limit < 1 || limit > 20 ||
        (project !== undefined && (typeof project !== "string" || !project.trim() || project.length > 200)))
    ...
    if (res.status === 404 || res.status === 405) {
      ...
      throw new Error("Brain upgrade required: this server does not support evidence search. Use direct record retrieval on older servers.");
    }
    if (!res.ok) {
      await res.body?.cancel();
    ...
    try {
      while (true) {
        ...
        if (raw.length > 20000) { await reader.cancel(); throw new Error("Evidence search response too large"); }
      }
      raw += decoder.decode();
    } finally { reader.releaseLock(); }
    let data;
    try { data = JSON.parse(raw); } catch { throw new Error("Invalid evidence search response"); }
    if (raw.length > 20000 || !Array.isArray(data?.sources) || data.sources.length > 20 ||
        data.returned !== data.sources.length || typeof data.truncated !== "boolean" ||
        data.sources.some(s => !s || typeof s.item_id !== "string" || typeof s.sid !== "string" ||
          typeof s.excerpt !== "string" || !Array.isArray(s.contributors)))
