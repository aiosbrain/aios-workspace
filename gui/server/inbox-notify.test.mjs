...
test("a later failed sibling cannot overwrite a successful delivery in the same tick", async () => {
  const repo = workspace();
  try {
    let calls = 0;
    const notifier = createTelegramNotifier({
      repo,
      loadLoop: async () => loopFor([askItem("ask-a-success"), askItem("ask-z-fail")]),
      env: configuredEnv,
      transport: async () => {
        calls++;
        return calls === 1
          ? { ok: true, status: 200 }
          : { ok: false, status: 401, description: "fixture-bot-value" };
      },
    });
