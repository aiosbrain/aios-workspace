...
| Reading or writing the AIOS Linear board (AIO-nnn issues) from this repo, or from a sibling repo that has no Linear tooling of its own | `.claude/skills/aios-linear/SKILL.md` — routing documentation for the canonical route: **`aios linear <verb>`**, the built-in adapter (`scripts/connectors/linear/`, AIO-1067). The adapter owns credential resolution (`LINEAR_API_KEY` env → user-level reference from `aios connect linear` → legacy workspace `.env` scoped decryption); missing config returns `AIOS_E_CREDENTIAL_MISSING` with the bootstrap command. `linear <cmd>` (the published bin, `scripts/linear.mjs`) and the skill-copy `linear.mjs` are warning-only delegates to the same adapter — identical stdout/exit, stderr deprecation, removal no earlier than v3.0.0. Do not `dotenvx run -f .env` the whole toolkit file (AIO-790). `scaffold/.claude/skills/aios-linear/` is the same routing content vendored into every scaffolded AIOS workspace via `aios update`; edit both copies together (scaffold/ is the operative source of truth). Byte-parity is enforced by `npm run check:linear-skill-parity` (AIO-927, `test:prepare` + CI `constitution` job — a thin gate over OGR17 `validation/check-skill-sync.mjs` plus a both-dirs-exist assertion) and by `test/linear-dotenvx-scope.test.mjs`; verb parity between the canonical route and the delegates is `test/linear-command-parity.test.mjs`. |
