...
| Reading or writing the AIOS Linear board (AIO-nnn issues) from this repo, or from a sibling repo that has no Linear tooling of its own | `.claude/skills/aios-linear/SKILL.md` — the canonical CLI. Primary route everywhere a global `@aiosbrain/aios` is installed: `linear <cmd>` (the PATH bin resolves `LINEAR_API_KEY` itself — SKILL.md's `LIN=linear`). From this repo `node scripts/linear.mjs <cmd>` is the same wrapper by path (decrypts only `LINEAR_API_KEY`); from a sibling repo run `node <path-to-aios-workspace>/scripts/linear.mjs <cmd>`. Do not `dotenvx run -f .env` the whole toolkit file — that decrypts unrelated secrets and emits key-mismatch warnings (AIO-790). `scaffold/.claude/skills/aios-linear/` is the same content vendored into every scaffolded AIOS workspace via `aios update`; edit both copies together (scaffold/ is the operative source of truth). Byte-parity is enforced by `npm run check:linear-skill-parity` (AIO-927, `test:prepare` + CI `constitution` job — a thin gate over OGR17 `validation/check-skill-sync.mjs` plus a both-dirs-exist assertion) and by `test/linear-dotenvx-scope.test.mjs`; deliberate divergence requires editing those checks. |
